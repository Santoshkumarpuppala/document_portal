...
class DocumentIngestion:
    ...
    def delete_existing_files(self):
        ...
        try:
            ...
            raise DocumentPortalException("An error occured while deleting existing PDF")

    def save_uploaded_files(self,refernce_file, actual_file):
        """
        ...
        try:
            ...
            with open(ref_path, "wb") as f:
                f.write(refernce_file.getbuffer())
            with open(act_path, "wb") as f:
                f.write(actual_file.getbuffer())

            self.log.info("Files saved", refernce=str(ref_path), actual_file = str(act_path))
            return ref_path, act_path
        except Exception as e:
            self.log.error(f"Error reading PDF: {e}")
            raise DocumentPortalException("An error occured while Uploading the PDF")
